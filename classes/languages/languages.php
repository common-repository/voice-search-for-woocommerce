...
class VswcLanguage
{
    ...
    const WIDGET_MESSAGES = 'vswcWidgetMessages';
    const PLACEHOLDER = 'placeholder';

    const ERRORS = 'vswcErrorLibrary';
     ...
     * 
     * @return Array $vswc_all_lang_set  Array of language arrays
     */
    public static function get_all_languages()
    {
        $vswc_all_lang_set = array();

        try {
            if (!empty(self::$gcp_language_set)) {
                $vswc_all_lang_set = self::$gcp_language_set;
            } else {
                $vswc_all_lang_set = array('en-US' => array(self::NAME => 'English (United States)', self::SHORT_PHRASE_LANG => 'en-US', self::LANG_CODE => 'en-US'));
            }

            uksort($vswc_all_lang_set, 'strcasecmp');
        } catch (\Exception $err) {
            $vswc_all_lang_set = array('en-US' => array(self::NAME => 'English (United States)', self::SHORT_PHRASE_LANG => 'en-US', self::LANG_CODE => 'en-US'));
        }

        return $vswc_all_lang_set;
    }
    ...
    public static function get_short_phrase_lang_code($lang_code)
    {
        ...
        $vswc_valid_lang_code = false;

        if (self::gcp_supported($lang_code)) {
            $vswc_lang = self::$gcp_language_set[$lang_code];
            $vswc_valid_lang_code = true;
        }

        if ($vswc_valid_lang_code === true && array_key_exists(self::SHORT_PHRASE_LANG, $vswc_lang)) {
            $short_phrase_lang = $vswc_lang[self::SHORT_PHRASE_LANG];
        }
