...
(function (window) {
    var WORKER_PATH = vswcWorkerPath;
    ...
    var Recorder = function (source, cfg) {
        ...
        var vswcAnalyserNode = source.context.createAnalyser();
        source.connect(vswcAnalyserNode);
        vswcAnalyserNode.fftSize = 2048;
        ...
        function vswcAdd(accumulator, a) { return accumulator + a; }

        // A function to calculate low pass filter value
        function vswcLowPassFilter(lastFilterValue, rawFreqValue) {
        ...
        function vswcMicButtonClickHandler() {
            try {
                if (typeof (vswcMicButtonElInCtx) != 'undefined' && vswcMicButtonElInCtx !== null) {
                    var vswcMicClassList = vswcMicButtonElInCtx.className || '';

                    if (vswcMicClassList.indexOf('listening') != -1) vswcMicButtonElInCtx.click();
                }
            } catch (err) {
                console.log('vswc Error: Unable to reset Mic button.')
            }
        }
        ...
        this.node.onaudioprocess = function (e) {
            ...
            var vswcAudioData = new Uint8Array(vswcAnalyserNode.frequencyBinCount);
            vswcAnalyserNode.getByteFrequencyData(vswcAudioData);

            //FIRST WE NEED TO ADD UP ALL FREQUENCIES TO GET THE TOTAL VOLUME
            var currentVol = 0;

            for (let i = 0; i < vswcAudioData.length; i++) {
                currentVol = currentVol + vswcAudioData[i];
            }

            lowFilterValue = vswcLowPassFilter(lowFilterValue, currentVol);
            ...
            // End mic recording 
            if (lowFreqCount > 17 && reachedHigh) { vswcMicButtonClickHandler(); }
        ...
        this.record = function (micButtonEl = null) {
            recording = true;
            vswcMicButtonElInCtx = micButtonEl;
        }
        ...
        vswcAnalyserNode.connect(this.node);
