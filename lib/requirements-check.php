 ...
 */
class Voice_Search_For_WooCommerce_Requirements_Check
{
    private $title = '';
    private $php = '5.2.4';
    private $wp = '3.8';
    ...
    public function deactivate()
    {
        ...
        //add_action( 'after_plugin_row_'.plugin_basename( $this->file ), array($this, 'chrome_only_plugin_inline_notice'));
    }

    public function chrome_only_plugin_inline_notice()
    {
        echo esc_html('<tr class="active"><td>&nbsp;</td><td colspan="2">
        Tareq
        </td></tr>');
    }
    ...
    public function php_version_notice()
    {
        ...
        <div class="error">
            <p>
                <?php printf('The &#8220;%s&#8221; plugin cannot run on PHP versions older than %s. Please contact your host and ask them to upgrade.', esc_html($this->title), $this->php); ?>
            </p>
        </div>
        <?php
    }
    ...
    public function wp_version_notice()
    {
        ...
        <div class="error">
            <p>
                <?php printf('The &#8220;%s&#8221; plugin cannot run on WordPress versions older than %s. Please update WordPress.', esc_html($this->title), $this->wp); ?>
            </p>
        </div>
        <?php
    }
